...
/**
 * Class for the customizer importer used in the Templatespare plugin.
 *
...
class AFTMLS_Customizer_Importer
{
   ...
   */
  public static function import_customizer_options($import_file_path, $import_customizer_options)
  {
    ...
    if (! file_exists($import_file_path)) {
      return new WP_Error(
        ...
        sprintf(
          esc_html__('The customizer import file is missing! File path: %s', 'templatespare'),
          $import_file_path
        )
      );
    }
    ...
    // Get the upload data.
    $raw  = AFTMLS_Helpers::data_from_file($import_file_path);
    ...
    if (! is_array($data) && (! isset($data['template']) || ! isset($data['mods']))) {
      return new WP_Error(
        ...
        esc_html__('The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'templatespare')
      );
    }

    // Import images.
    if (apply_filters('templatespare/customizer_import_images', true)) {
    ...
    if (isset($data['options'])) {
      ...
      require AFTMLS_PLUGIN_DIR . 'includes/companion/class-aftc-customizer-option.php';

      foreach ($data['options'] as $option_key => $option_value) {
        $option = new AFTMLS_Customizer_Option($wp_customize, $option_key, array(
          'default'    => '',
          'type'       => 'option',
          'capability' => 'aftct_theme_options',
        ));
    ...
    $use_wp_customize_save_hooks = apply_filters('templatesapre/enable_wp_customize_save_hooks', false);
    ...
    foreach ($data['mods'] as $key => $val) {
      if ($import_customizer_options !== true) {

        if ($key === 'nav_menu_locations' || $key === 'sidebars_widgets') {
          continue;
        }
      }
      if ($use_wp_customize_save_hooks) {
